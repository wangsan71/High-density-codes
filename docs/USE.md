...
- **新的高密度纸面档 `P-MX-300-4/5/6`**：纸面模块改为**实心方块/空白**，不再画参考环；timing 行与较大的回显模块负责定位和黑白校准。模拟扫描固定 16 seed 下三档都是 16/16 逐字节还原，但它们**仍标为「新档·真机待验」**，不会冒充真扫描证据。不可压 1MB 的页数从默认 `P-M1-300` 的约 169 页降到约 **64 / 45 / 29 页**。用法：
