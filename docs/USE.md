...
**手机接收该打印哪一种档？—— 实测答案（第 80 轮）**：整张 A4 塞进一张手机照片时，**纸面档 `P-M1-300` 的格子只剩约 2.7 px（符号错 72.83%、0/8 还原）**，而**粗档板材 `PL-G@0.4` 的格子还有约 15 px（8/8 逐字节还原，裁切生效时 6/8）**。所以：**手机那一端请用 `PL-G`（发送页下拉框里带「手机拍摄首选」标记的那个），或者把手机拍近到码区填满画面**。纸面档的正确用法是**扫描仪 300 dpi**（那一档实测 200/200 ✓）。
...
- **用手机拍纸面页，一页都读不出**：这多半**不是你的错**。实测（第 79–80 轮，`tools/g4-probe.ps1`）：整张 A4 塞进一张手机照片时，纸面档 `P-M1-300` 的格子只剩约 **2.7 px**（符号错 72.83%），而粗档板材 `PL-G@0.4` 的格子还有约 **15 px** ⇒ **同一套取景下纸面 0/8、板材 8/8 逐字节还原**。三条出路：① 用**粗档**（`PL-G` 或其它板材档）做手机那一端；② **拍近**（让码区填满画面，别把整页塞进去）；③ 改用**扫描仪 300 dpi**。工具现在会区分原因：说 `no-contrast` 就是曝光/反光/太远，说 `echo-no-contrast` 就是「条带（页上最细的特征）糊了，拍近或换粗档」——**都不是让你去重印**。
