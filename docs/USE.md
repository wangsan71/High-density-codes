...
| 项 | 怎么做 | 现状 |
|---|---|---|
| **G4 手机压力** | 电脑上 `node tools/serve.mjs`（详见 §2）→ 手机连**同一个 Wi-Fi**、打开它打印出来的那行局域网地址（接收页）→ 拉到页面最下面那一节「**手机连拍（自动挑帧）**」按「**开始连拍**」（**不是**上面第 1 节那个「用摄像头拍一页」——那个是一次一页的手动快门，500 页要按 500 次 ✗）→ 连拍 **500 页 × 8 轮**，看是否 100% 还原；失败就把接收页点名的缺页号与它给的重拍提示一起发回 | **从未跑过** ✗ |
