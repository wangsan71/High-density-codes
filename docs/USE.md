...
它生成：纸面页（PNG + 真尺寸 pack.pdf）、**P-MX-300-6/5/4 三个高密度模块纸面 leg**、**每个喷嘴一块码牌**（`PL-G` @ 0.2/0.4/0.6/0.8 + `PL-D2` @ 0.4，载荷 6 B ⇒ **每块板只打 `page-000`**）、MTF 校准板，以及一份 **`README.txt`**（逐步命令 + 每步在验什么 + 失败时该发回什么）。它自己会把每个 `.3mf` 过一遍 G8 子集校验，**任何一个不合格就 exit 1**（实测：改一个字节 ⇒ exit 1，原文件 ⇒ exit 0）。它**不假装**验过硬件 —— 打印、扫描、拍照、开浏览器那几步是你的，README 里写清了；其中的模块纸面一步正是把模拟 16/16 推进到真平板扫描的必要验收。
