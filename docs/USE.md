...
- **扫描**：**用 300 dpi**（这一档实测达标：200 seed × 3 页 ⇒ **200/200 逐字节还原**、门限 G2 的 300 dpi 侧 `exit 0` ✓）、**彩色**、**关掉自动裁剪/去边界**（会切掉角上的基准标记 ✗）。**600 dpi 先别用**：同一判据、同一载荷、同一信道口径（`nocrop`）下实测 **162/200 = 81%**，而且**页级直接读出只有 43%**（162 份"成功"里有 145 份是靠页间冗余把读不出的页兜回来的）⇒ 记 **D49 OPEN** ⇒ 在 `pskit calibrate` 落地、或 `P-M1-600` 这一档的页内 ECC 余量重做之前，**600 dpi 不是可靠路径** ✗ 相应地：CLI 里 `--profile P-M1-600` 仍然可选（**网页发送端不提供这一档** ⇒ 照本手册用网页的人踩不到 ✓），但**照本手册走就不要选它**；600 dpi 的失败形态是"缺页/读不出并点名要补拍"，**不会**交出错误字节（误接受 0 是硬约束 ✓）。
...
- **想自己验内核**：`node cli/pskit.mjs verify --gate all`（G0 单测 + G1/G3/G5/G7/G8 进程内），或接收页 `?selftest=1`。单测条数每轮都在长 ⇒ **别背数字**：第 52 轮实测 **296/296**、第 53 轮加了纸面斜拍 1 例后 `exit 0`（当轮我没抄下汇总数字 ⇒ 就不写没量过的数 ✗）⇒ 以 `node --test --test-isolation=none "tests/unit/**/*.test.mjs"` 自己打印的汇总为准，**只看 exit code 判定** ✓
