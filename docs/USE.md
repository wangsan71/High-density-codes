...
接收页：把**整盘**扫描或照片一次拖进去（可以乱序、可以缺页，页面会报缺哪几页；页间有 RS 冗余，少量缺页能自动补回）→ 还原后落盘。**存成什么名字可以自己填**（可选输入框，桌面接收与手机连拍两处都有）：页头**没有文件名字段**（只有摘要），所以留空就用从字节导出的默认名 `pskt-<字节数>B-<摘要前12位>.bin`；**手机上想直接打开，就填回原来的名字带扩展名**（例如 `report.pdf`；只填 `.pdf` 也行 ⇒ 把默认名的 `.bin` 换成它），因为 iOS/Android 靠扩展名决定用什么打开文件。填的名字**只在你本机用于命名下载**，不发送、不进页码、不参与校验（第 69 轮加，D62）。
...
| 项 | 怎么做 | 现状 |
|---|---|---|
| **G4 手机压力** | 电脑上 `node tools/serve.mjs`（详见 §2）→ 手机连**同一个 Wi-Fi**、打开它打印出来的那行局域网地址（接收页）→ 拉到页面最下面那一节「**手机连拍（自动挑帧）**」按「**开始连拍**」（**不是**上面第 1 节那个「用摄像头拍一页」——那个是一次一页的手动快门，500 页要按 500 次 ✗）→ 连拍 **500 页 × 8 轮**，看是否 100% 还原；**每轮收齐后在「存成什么名字（可选）」里填回你发的那个文件名带扩展名**（例如 `book.epub`；留空会下载成 `pskt-…B-….bin`，手机上没有扩展名就**打不开**——那不是解码失败，是命名，D62）→ 打开它确认内容对；失败就把接收页点名的缺页号与它给的重拍提示一起发回 | **从未跑过** ✗ |
