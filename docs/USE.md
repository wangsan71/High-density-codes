...
- **瓦片页（P4，像 QR 一样"认住任意一块即可"的新路子，第 160–167 轮）**：整页切成 M×N 块瓦片，每块自带三实心 + 一空心定位图形，载荷**分片**铺进去（每块带 2 字节头 + 2 字节长度）。一条命令写页、一条命令读回：
  ...
  **目前能做什么、不能做什么（别高估）**：能写页、能从**自己渲染出来的 PNG** 逐字节读回、能容忍**平移**（±12 px 内搜到、分辨率约一个模块）；**读取端第 237 轮起能读 5° 斜拍照片**（实测：四角偏差 ≤40 px 都能整页 48/48 逐字节读回、60 px 具名拒读），但**没有任何命令或网页入口调用它** ⇒ **用户侧仍然拿手机拍不了瓦片页**。旋转（整页 90°）与光照不均仍未做；瓦片级纠错（RS）也还没接。
