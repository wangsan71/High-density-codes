...
- **扫描的格式**：**存成 PNG**。**黑白（1-bit）/ 灰度 / 彩色 / 调色板 / 16-bit 灰度都直接支持**（第 95 轮 D80 起：扫描仪的"黑白·线稿"默认档就是 1-bit 或调色板 PNG，以前会被拒 ⇒ 现在四种都实测逐字节还原，其中 1-bit 反而是最干净的输入）。CLI 只解码 PNG 这一个**容器**（TIFF/JPEG/PDF 会明确告诉你「这个格式读不了」并给出转换命令）；手机拍照那条路走浏览器接收端时不受此限（浏览器自己解 JPEG）。**特别提醒"扫描成 PDF"**：那是最多扫描仪的默认输出，而本工具**写得出一份 PDF（打印路径）却读不了一份** —— 目录里只有 `.pdf` 时它现在会直说这一点并让你导出 PNG（第 93 轮 D79），**不要靠截屏**：截屏会重采样墨点、把角标挪位置。
