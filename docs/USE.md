...
- **扫描**：**用 300 dpi**（这一档实测达标：200 seed × 3 页 ⇒ **200/200 逐字节还原**、门限 G2 的 300 dpi 侧 `exit 0` ✓）、**彩色**、**关掉自动裁剪/去边界**（会切掉角上的基准标记 ✗）。**600 dpi 先别用**：同一判据、同一载荷、同一信道口径（`nocrop`）下实测 **162/200 = 81%**，而且**页级直接读出只有 43%**（162 份"成功"里有 145 份是靠页间冗余把读不出的页兜回来的）⇒ 记 **D49 OPEN** ⇒ 在 `pskit calibrate` 落地、或 `P-M1-600` 这一档的页内 ECC 余量重做之前，**600 dpi 不是可靠路径** ✗ 相应地：CLI 里 `--profile P-M1-600` 仍然可选（**网页发送端的下拉框同样会列出它** ⇒ 我第 58 轮写的"网页不提供这一档"是**错判、第 60 轮已撤回** ✗ 那个下拉框是 `Object.entries(PROFILES)` 动态建的、档位 id 从来不以字面量出现，所以"grep 不到"证明不了"不提供"。第 60 轮起该选项的标签带 **`⚠ 实测未达标 (D49)`**，但**它仍然可选** ⇒ 别选），但**照本手册走就不要选它**；600 dpi 的失败形态是"缺页/读不出并点名要补拍"，**不会**交出错误字节（误接受 0 是硬约束 ✓）。
