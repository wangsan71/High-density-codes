...
发送页：拖入或选择文件 → 选 profile（纸面 `P-M1-300`；实体码牌 `PL-D2`）→ 下载 **页图 PNG（按钮写着「PNG（zip）」：所有页打在一个 zip 里、一次下载完 ⇒ 不必跟浏览器的"批量下载拦截"打交道；里面每页一张 PNG，同样是整张纸：码区居中 + 裁切/套准标记 ⇒ 第 52 轮起，D45 已闭）** / **`pack.pdf`（页 = 纸张、码区按真实物理尺寸居中、含裁切与套准标记，直接送打印机）** / 板材 **`.3mf` + `.stl`**。
