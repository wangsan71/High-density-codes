# 交接文档 · PSKT（第 100 轮末 · 骨架写于第 73 轮末、逐轮更新 · **无 tag**）
...
>
> **第 100 轮状态**：`module` 物理编码与 `P-MX-300-4/5/6` 已接入硬件验收包。
> ① 模拟扫描三档 16/16；② `acceptance-kit.ps1` 现在生成三份模块 pack.pdf 和真平板扫描命令；③ 默认档仍 `P-M1-300`；
...
2. **`P-MX-300-4/5/6` 真机待验**：三档均通过固定 16 seed 的 `sim/channel.py --preset scan300 --modifier nocrop`，
   G1 理想往返 0 错；`acceptance-kit.ps1` 已给出逐档打印/扫描/接收命令，但真实平板扫描仍未执行。
...
**进程内（若你不能跑硬件，按价值排序）**：
1. **真平板扫描验证 `P-MX-300-6/5/4`**：16/16 模拟已完成；`acceptance-kit.ps1` 已把三份页与命令打包，先打 6px 一页。
