...
**第 188–225 轮（瓦片页的"照片那一层"，尚未打通 —— 下一个人从这里接手）**：这条线一共 23 轮，其中**撤回 10 次**，但每轮都留下硬事实，**全部记在 `docs/STATUS.md` 第 188–225 轮块里**，这里是浓缩版：
...
- **卡在哪**：把"粗拟合 → 块号身份校正 → 逐块读数精锚点 → 最小二乘单应"拼起来时，**角点诊断能到 ≤25 px（221 轮）甚至 (522,31)（220 轮），但都没让最终整页读取通过**；第 225 轮拼法更是与第 216 轮**逐位相同**，说明某两步互相抵消。**下一步只需打印三个数**：`refined.length`、精锚点平均偏移、单应 RMS —— 三个数就能分开"没搜到"与"搜到但没改善"（225 轮已写明）。
...
- **一个产品层面的判断（留给产品负责人或下一轮的我）**：这条线是"手机斜拍也能收"的**增强**，不是"能用"的**前提** —— 瓦片页的写/读/纠错/平移/剪切都已可用，而且用户手册（`docs/USE.md`）已写明"**不能从照片读**"。⇒ 继续攻透视、还是先把瓦片路径接进手机端与 CLI 的正式流程，**是一个可以商量的取舍**。
