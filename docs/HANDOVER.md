...
| G9 Web 扫描端 | 🟡 | 产物级全绿（顶层 13 项 + `G9 CHECK` **15** 断言、零第三方加载点、CSP、SW 清单哈希对上磁盘、bundle 盲解磁盘页摘要相符）；**第 264 轮真浏览器把 `file://` 单文件版也验了**（逐字节还原 1,500 B、摘要独立复核一致）⇒ **差你自己点一次**（真人点击 + Edge / Safari）。**D43 的 https 半边第 90 轮已闭**（站点见 §14） |
