...
**第 188–225 轮（瓦片页的"照片那一层"，第 237 轮**库内**已打通、**仍缺照片入口** —— 下一个人从这里接手）**：这条线一共 23 轮，其中**撤回 10 次**，但每轮都留下硬事实，**全部记在 `docs/STATUS.md` 第 188–225 轮块里**，这里是浓缩版：
...
**还差的**：① **照片入口**（第 237 轮起**读取端本身能读 5° 斜拍照片**了 —— refine + 块号投票默认开、四角误差 ≤40 px 都能救回；但**全仓库没有任何 CLI/网页入口调用它**，所以「用户拿手机对着纸扫」这一步**仍然不成立**）② 瓦片级 RS 纠错（现在只有 CRC 检测）
