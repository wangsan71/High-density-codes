...
| 提交 | 轮次 | 做了什么 |
|---|---|---|
| (本轮) | 94 | **台账收口**：D51 的状态列头上还写着"未修"（第 63 轮就落地了）⇒ 用 D51 自己点名的两条命令复验：`level-diff` 首读**仍**塌陷（设计如此）、`receive` 把整条传输**救回且逐字节相同** ⇒ 改记 CLOSED 并纠正当时写错的验收形态；同时启动 600 dpi 的 G2 重跑（D49 的最终判决）|
