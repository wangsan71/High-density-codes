...
**用户侧（价值最高，只有你能做）**（**本段第 296 轮刷新 —— 旧的「打 MTF 板与四喷嘴码牌」已随 3D 线退役 ✗，别再照旧版做** ✓）：
⓿ **`git push origin master`** —— 本地领先 `origin/master` **198 个提交**（第 294 轮实测 ✓）⇒ **不推，线上站点就还是旧的** ✓；推完照 `docs/USE.md` §5 那三步确认（标题 / `?selftest=1` / Actions 页 ✓）。
① `& .\tools\acceptance-kit.ps1` 一次生成**纸面页 + 三档模块页 + 三张密度梯页 + `README.txt`** ✓（**3D 板材码牌与 MTF 板已随 3D 线退役移除** ✓ 别去找）⇒ 照 README 走四步 ✓；它的产物**已自证**（第 275/281 轮：纸面、`module-6`、最密的 `module-4`、以及密度梯过信道后**逐字节相同** ✓）。
② **G4 手机**（闭合 M4 的唯一动作 ✓）→ ③ **G9 三浏览器**（**Safari 在 Windows 上不存在** ✓、**Edge 在本沙箱起不来**（crashpad 被拒 ✓）⇒ 这两格只能你做 ✓）→ ④ **D8 打印缩放** ✓。

**进程内（若你不能跑硬件；剩下很少）**：
1. **真平板扫描验证 `P-MX-300-6/5/4`**：模拟侧已 100% ✓、验收包已打包 ✓、`check-module-scans.mjs` 可一键核对 ✓（**目录名按 README 的 `scans-module-N`** ✓，工具两种命名都认 ✓ D97）；若只能拍照，用默认 `P-M1-300` ✓；**不要用手机照片把模块档记成通过** ✓。
2. **待产品负责人决定的三件**：600 dpi 判据（第 63 轮已明示非必要 ✓）、**瓦片路径是否接进 `pskit send/receive`**、**网页分片 UI** ✓ —— **不要自行动手** ✓。
3. **不建议做**：为 600 dpi 再优化候选排序（已量到地板 ✓）；3 h 长跑（判据只要 30–60 min ✓）；任何「为了让门限表好看」的判据收窄 ✓；为了 `phone-full` 预设去放宽角标判读（D92 = NOTABUG ✓）。
4. **已经验证过、不要再查的**（详见 `docs/DONE.md` 的 A/C 两节 ✓）：`#outname` 命名接线（第 287 轮真 DOM 验过 ✓）、`?selftest=1`、SW 离线（真断网 ✓）、TIFF（含多页 ✓）、`file://`、手机四要素组合、**Pages 子路径形态**、连拍管线、Windows 的 JPEG→PNG 退路 ✓。
