...
| G9 Web 扫描端 | 🟡 | 产物级全绿 + **第 249–251 轮真浏览器（服务版 http）已跑通发送/接收两条链、`?selftest=1` 13/0、SW 缓存字节与磁盘一致**（第 254–256 轮补：加密三态、命名规范化、**单文件版两个产物**也各跑通一次、摄像头被拒文案）；**仍差**：可见浏览器（无真人点击）、Edge / Safari、`file://` **双击**这一格（协议本身，影响 SW/摄像头而不影响解码）、手机 |
