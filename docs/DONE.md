...
9. **死代码复查是三条腿，且现在是命令**（第 262 轮定型）：① 导出侧 `node tools/find-dead-exports.mjs` ⇒ 长期停在 **3**（`expectedRho` / `decodePages` / `DEFAULT_PROFILE`，全是「文档即接口」）⇒ **不要再当待办**；② 模块级非导出孤儿、③ **孤儿工具文件** —— ②③ 都在 `node tools/find-dead-locals.mjs`（`--selftest` 是阳性对照）⇒ **不要再用 `.tmp/` 里的探针做复查**（第 257 轮那个探针既违反 AGENTS §6.8、又会随会话消失，第 262 轮才补成命令）。**注意词边界**：用 `\b` 会数不到 `$(...)` 这类调用（`$` 不是单词字符），`web/app.js` 的 `$` 就是这样被误报过一次。
