...
- 交叉校验：`python ref/decode.py` 从 `58 FAIL` 降到 **12 FAIL / 5 组**，且剩余每一项都已定位到具体一行（见变更日志 M4 下半之二）。**除了 `page-unpack-plain-PL-D2`，剩下的全是参考侧的 bug**——`rs-erase 0/4` 与它引发的 `transfer-loss 0/3`、`meta` 那条擦除自检（`python ref/probe_rs.py` 给最小复现：2 错已能精确纠正，3 擦除仍报 "no codeword"）；`deflate-deflate-mixed` + `deflate-empty` 是同一处容器偏移（它把 PSZ1 体的起始位置算错一字节：`deflate-raw 6/6` 全过证明我的 fixed-Huffman 流本身没问题，报 "invalid stored block lengths" 只可能是喂进去的字节不对）。`page-unpack` 那条是**唯一可能真分歧**的：content 前 220 字节完全一致、parity 后半不一致 → `unequal` 模式次级(形状)通道的位序/位宽；下一轮先判谁错。跑绿之前 G2/G4 不能拿它当门限证据。
