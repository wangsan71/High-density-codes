...
- 交叉校验：`python ref/decode.py` 现在 **只剩 12 FAIL 且全在一类**——所有原语、GF(2⁸)/RS 编码、纠错、擦除、交织、CRC、头 CRC、容器、色道解包、单页解码**逐组全绿**（`meta 12/12 crc 11/11 sha256 25/25 deflate 10/10 deflate-raw 6/6 chacha20 8/8 pbkdf2 6/6 rs-encode 12/12 rs-erase 4/4 rs-error 4/4 rs-fail 4/4 interleave 40/40 header-crc 2/2 page-unpack 21/21 page-decode 15/15 geometry 40/40`）。剩下：`transfer 43/50`+`transfer-loss 0/3`（同一处：干净页端到端重组后**只有 offset 1 起差一字节**，两边各自都能自证 CRC/摘要，所以是行为分歧而非笔误；已排除的假设：intra 块排布——两边逐行对过，完全一致）与 `header 28/30`（它的 `h_header` 在查 `magic` 键时 KeyError；同一份头在 transfer 里解析正常）。下一轮先用"两边对同一页各自算 intra 伴随式"两步定位那一个字节。跑绿之前 G2/G4 仍不得引用它作证据。
