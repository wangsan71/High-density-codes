...
| G2 | 纸面 300/600dpi 200 seed 100% | **首次接上真实物理信道即失败：0/3 页读出**。`python sim/channel.py --in .tmp/g2src --out .tmp/g2scan --seed 7 --preset scan300` → `pskit receive --photo` 三页全报 `markers/blank-image`/`no-hollow-corner`。信道报告本身可信（`mean_coverage 0.25` = 25% 着墨、旋转 0.77°、模糊 σ 0.076 mm、JPEG q 87、glare、纸纹、白平衡），所以**"blank-image" 是我的二值化/基底白估计被真实光照分布打穿了**，不是文件坏了。此前 G1/G7/"字节穿越照片"全部只跑在我自己写的合成畸变上——**这正是 G2 存在的意义，它一上线就抓到全项目最重要的一类自欺**。下一步：把 `binarize` 的白参考估计改成不依赖"边框一定是背景"（信道 `frame_px` 比页略大但 `crop_frac` 仅 0.008，边框几乎全是页面自身），并加一张失败页的直方图落盘工具。修好之前 G2 记 **未通过**。 |
...
- `sim/channel.py` 的代理长跑未收口（81 KB + selfcheck 60 KB、无 README），已下命令要求"先写 README + 保证一条真实命令跑得通，再回简报，不要再加退化模型"。G2（纸面 300/600dpi × 200 seed）与 G4（手机压力档）等它交付那条命令；若下轮仍无可用出口，就**我自己接手 `sim/`**（M5 两次委托零产出已经是前车之鉴）。
