...
- **角标（第 77 轮，D68）**：尺寸**不发明**，全部取自 `pageLayout()` 的 `layout.fiducials`（像素）按
  `mmPerPx = 25.4/dpi` 换算 —— 中心 `(f.x, f.y)`、半边 `f.half`、环宽 = 该标记自己的 `f.ringPx`
  （页面布局给的是 `max(cellPx, ewPx)` ⇒ **≥1 EW**，可打印；薄于一格 EW 时装配**直接拒绝**，不放宽成
  "差不多能印"）。y 轴与格子同一套翻转（`y_model = pageH_mm − y_image`）。空心角标 = 外方框减内方框，
  用 `rectilinear.js` 挤出（逐对象水密）。**没有角标的板材照片登记不了** ⇒ 这不是装饰，是可用性。
...
3. **投影对拍（最关键的一条）**：把 3MF 顶面三角形投影到 Z=顶面平面，按 `pitchMm` 网格统计每格
   ...
   这条把"网格能打开"升级为"网格印出来是同一张码"。**四角标记（第 77 轮）**：它们按设计在点阵之外，
   只有 `facts.markers` 声明了位置与期望面积时才从"点阵外材料"里排除；**每个被声明的标记必须真的在、
   面积对得上（同样 < 8%）**，否则"排除"就成了判据上的洞。两侧同一条规则：JS 的
   `projectTopToCells({markers})` 与 `ref/verify_model.py` 的 `g8-3/marker-material`；判定框的容差是
   **1 µm**（待判网格是焊接过的、坐标 6 位小数，顶点可能落在声明框外 ≤5e-7mm —— 实测 1e-9 会让两个
   标记读成 0 mm²、并把 10 个三角形报成跨界）。
